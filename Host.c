...
double Hh = 1375,Rr = 1000., Ll=2500,Vv=0;
struct dis_set  ds_vol[]=
{
//-------------------
  &Hh,
  0,
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"FTCT" ))
        {   //'FTCT'
         if( f_dis_set(ds_ftct,4,3) > 0)
         {
           f_prep_VM(0);
           f_prep_VM(1);
           f_prep_VM(2);
...
//    printf("\n i=%d ,i1= %d,count =%ld",i,i1,count);
    ...
    /*----------------------------------------------*/
      if (!strcmp(intrpr.wrd,"PRC" ))
       {   //'PRC'
          f_dis_set(ds_prc,3,3);
          goto fin;
       }
    /*----------------------------------------------*/
/*
      if (!strcmp(intrpr.wrd,"DCYL2" ))
         // -----  display Cyl Table 2 ---
        {
          i=1;
          goto m_DCYL;
...
m_DCYL:
           ...
           for(;i1<i2;i1++)
           {
            printf(" %10g %10g %10g\n\r",cyl_uk[i1].Radius,cyl_uk[i1].Lgth,cyl_uk[i1].Offs);
           }

          goto fin;
           }
    //-----------------------------------------
          if (!strcmp(intrpr.wrd,"CYL2" ))
         // -----  receive Cyl Table 2 ---
           {
           i=1;
           goto m_Cyl;
  ...
  m_Cyl:
             ...
             if(!f_inw()) { goto fin_e; }
             if(f_innum(intrpr.wrd) ) i2 = intrpr.num_var;
             else  { goto fin_e; }

             n_Cyl[i]=i2;

             if ( i2 >= cyl_nn)  { goto fin_e; }

             for(i1=0;i1<i2;i1++)
             {
                 f_get_wrd();
                 if(f_innum(intrpr.wrd) )
                  { // запись очередного значения Radius
                    cyl_uk[i1].Radius=intrpr.num_var;
                  }
                 else   goto fin_e;
                 ...
                 if(f_innum(intrpr.wrd) )
                  { // запись очередного значения Radius
                    cyl_uk[i1].Radius1=intrpr.num_var;
                  }
                 else   goto fin_e;

                 f_get_wrd();
                 if(f_innum(intrpr.wrd) )
                  { // запись очередного значения Lgth
                    cyl_uk[i1].Lgth=intrpr.num_var;
                  }
                 else   goto fin_e;

                 f_get_wrd();
                 if(f_innum(intrpr.wrd) )
                  { // запись очередного значения Offs
                    cyl_uk[i1].Offs=intrpr.num_var;
                  }
                 else   goto fin_e;
             }
//-----------------------------
...
//---------------------------------
      if (!strcmp(intrpr.wrd,"VOL" ))
        {   //'VOL'
          ...
          if( f_dis_set(ds_vol,3,3) > 0)
          {
            Vv=f_vol_cyl( Hh,  Rr, Ll) ;
          }
          printf( "  V = %lf m3",Vv);
          goto fin;
        }
