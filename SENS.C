...
//  status == 2  ; без уровня воды
//  CRC_flag == 0; - использовать значения плотности жидкой фазы из прокачки
...
// COM  19200 8 0 1
int State_PMP[max_PMP+1]={0,0,0,0};
long int PMP_ADDR[max_PMP]={1,2,3};
float Lvl_offs[max_PMP]={0,0,0};
...
//   n_bcom[ii]=strlen(cb_COM[ii]);
 ...
 if(buf[20] != 0xf0  ) goto m_err;
            ...
            s_DGM[nn].Prod_lvl = f_get_float_PMP(&buf[5])* 1000. + Lvl_offs[nn];
            s_DGM[nn].Temp     = f_get_float_PMP(&buf[9]);
//          s_DGM[nn].DensReal  = f_get_float_PMP(&buf[13])* 1000.;

           if(PMP[nn].CRC_flag != 0)
             s_DGM[nn].DensLiq   = f_get_float_PMP(&buf[13])* 1000.;


            s_DGM[nn].Wat_lvl   = f_get_float_PMP(&buf[17])* 1000.;
//        s_PMP[nn].Wat_lvl   = f_get_float_PMP(&buf[17]);
   //       State_PMP[nn]= f_get_long_PMP(&buf[21]) & 0xffff;
            State_dgm[nn]= f_get_long_PMP(&buf[21]) & 0xffff;

            if(PMP[nn].status== 2) s_DGM[nn].Wat_lvl=0;
